...
def state_to_tensor(state):
    ...
    return (torch.Tensor(state[0]).unsqueeze(0), torch.cat(
        (torch.from_numpy(state[1].copy()), torch.from_numpy(state[2].copy())),
        2).permute(2, 1, 0).float().div_(255).unsqueeze(0))
